...
CHANNEL = "BHZ"  # select "*HZ" or "*HN, *HE" for vertical or horizontals, respectively. "*DF" for infrasound.

# If using for arrays, just select a source lat/lon and radius that will envelope the array.
SOURCE_NAME = "Spurr" 
...
inv = client.get_stations(latitude=SOURCE_LAT, longitude=SOURCE_LON, maxradius=kilometer2degrees(MAX_RADIUS),
                          network="*", station="*", location="*", channel=CHANNEL, starttime=STARTTIME,
                          endtime=ENDTIME, level="response")

st = gather_waveforms_bulk(lon_0=SOURCE_LON, lat_0=SOURCE_LAT, max_radius=MAX_RADIUS, network='*',
                           station='*', location='*', channel=CHANNEL, starttime=STARTTIME, endtime=ENDTIME, n_jobs=4) # change n_jobs to use additional/fewer CPU cores.
...
fig, axs = plotting.plot_network_coherence(Cxy2_net, data, cmin=0.35) # change cmin here depending on coherence noise floor.

#%% Retrieve all station-pair coherograms
